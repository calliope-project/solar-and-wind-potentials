...
def land_cover_stats_to_csv(path_to_units, path_to_land_cover, attributes, path_to_results_csv):
    ...
    #ipdb.set_trace()
    units = gpd.read_file(str(path_to_units)).set_index('id')

    cmap = {int(attribute.split("_")[1]): attribute for attribute in attributes}
    stats = zonal_stats(str(path_to_units), str(path_to_land_cover), categorical=True, category_map=cmap)
    stat_df = pd.DataFrame(data=stats, index=units.index)
    stat_df.to_csv(str(path_to_results_csv))
...
if __name__ == "__main__":
    land_cover_stats_to_csv(
        ...
        path_to_results_csv=snakemake.output
    )
